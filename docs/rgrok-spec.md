...
pub enum ClientMsg {
    ...
    Auth {
        token: String,              // JWT
        version: String,            // control protocol version (must match server)
    },
...
# Public IPv4 used for per-tunnel Cloudflare A records (required when enabled)
...
# Set to true to create/delete DNS records per-tunnel (requires server.public_ip)
# Set to false if you use a wildcard *.tunnel.mysite.com A record (recommended)
# Config validation rejects true without server.public_ip and DNS credentials.
